...
def main(args):
    ...
    *_, func_args = inspect.getargvalues(inspect.currentframe())
    ...
    if config.pretrained_unet_path:
        ...
        unet = UNet3DConditionModel.from_pretrained(config.pretrained_model_path, subfolder="unet")
    ...
    poseguider = PoseGuider(noise_latent_channels=4).from_pretrained(config.pretrained_poseguider_path)
    ...
    referencenet.to(torch.float16)
    poseguider.to(torch.float16)
    clip_image_encoder.to(torch.float16)
